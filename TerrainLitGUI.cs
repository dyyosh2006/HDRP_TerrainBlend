...
namespace UnityEditor.Rendering.HighDefinition
{
    ...
    class TerrainLitGUI : HDShaderGUI, ITerrainLayerCustomUI
    {
        ...
        private class StylesLayer
        {
            ...
            // sheep yang
            public readonly GUIContent heightTransition = new GUIContent("高度混合权重", "0为不混合，1为全混合");
            public readonly GUIContent tilingfixedMode = new GUIContent("Tiling混合模式", "Off不开启，Notiling为开启");
             public readonly GUIContent useTriplanar = new GUIContent("使用三向UV", "Off不开启，UseTriplanar为开启");
             public readonly GUIContent useTriplanarWorldScale = new GUIContent("三向UV世界缩放值", "缩放三向UV大小值");
             public readonly GUIContent useTriplanarBlend = new GUIContent("三向UV融合权重", "越大颜色越接近，性能越耗");
             public readonly GUIContent usePixelDisplacement = new GUIContent("使用POM", "Off不开启，UsePOM开启");
             public readonly GUIContent usePixelDisplacementHeight = new GUIContent("POM高度值", "越大越高");
             public readonly GUIContent usePixelDisplacementSteps = new GUIContent("POM迭代步数", "越大效果越好，性能越耗");
        ...
        //sheep yang
        MaterialProperty tilingfixedMode = null;
        MaterialProperty useTriplanar = null;
        MaterialProperty useTriplanarWorldScale = null;
        MaterialProperty useTriplanarBlend = null;
        MaterialProperty usePixelDisplacement = null;
        MaterialProperty usePixelDisplacementHeight = null;
        MaterialProperty usePixelDisplacementSteps = null;
        enum TilingfixedMode
        {
            Off,
            NoTiling
        };
        ...
        protected void FindMaterialProperties(MaterialProperty[] props)
        {
            ...
            foreach (var prop in props)
            {
                ...
                else if (prop.name == ktilingfixedMode)
                    tilingfixedMode = prop;
                else if (prop.name == kUseTriplanar)
                    useTriplanar = prop;
                else if (prop.name == kUseTriplanarWorldScale)
                    useTriplanarWorldScale = prop;
                else if (prop.name == kUseTriplanarBlend)
                    useTriplanarBlend = prop;
                else if (prop.name == kusePixelDisplacement)
                    usePixelDisplacement = prop;
                else if (prop.name == kusePixelDisplacementHeight)
                    usePixelDisplacementHeight = prop;
                else if (prop.name == kusePixelDisplacementSteps)
                    usePixelDisplacementSteps = prop;
            ...
            using (var header = new MaterialHeaderScope(styles.header, (uint)Expandable.Terrain, materialEditor))
            {
                if (header.expanded)
                {
                    if (enableHeightBlend != null)
                    {
                        ...
                        tfm = (TilingfixedMode)mat.GetInt(ktilingfixedMode);
                        tfm = (TilingfixedMode)EditorGUILayout.EnumPopup(s_Styles.tilingfixedMode,tfm);
                        if (EditorGUI.EndChangeCheck())
                        {
                            switch (tfm)
                            {
                                case TilingfixedMode.Off:
                                    mat.SetInt(ktilingfixedMode, 0);
                                    mat.DisableKeyword("_NOTILE");
                                    break;
                                case TilingfixedMode.NoTiling:
                                    mat.SetInt(ktilingfixedMode, 1);
                                    mat.EnableKeyword("_NOTILE");
                                    break;
                            }
                        }
                        ...
                        EditorGUI.indentLevel++;
                        EditorGUI.BeginChangeCheck();
                        ut = (UseTriplanar)mat.GetInt(kUseTriplanar);
                        ut = (UseTriplanar)EditorGUILayout.EnumPopup(s_Styles.useTriplanar,ut);
                        if (EditorGUI.EndChangeCheck())
                        {
                            switch (ut)
                            {
                                case UseTriplanar.Off:
                                    mat.SetInt(kUseTriplanar, 0);
                                    mat.DisableKeyword("_TRIPLANAR");
                                    break;
                                case UseTriplanar.UseTriplanar:
                                    mat.SetInt(kUseTriplanar, 1);
                                    mat.EnableKeyword("_TRIPLANAR");
                                    break;
                            }
                        }
                        if(ut == UseTriplanar.UseTriplanar)
                        {
                            EditorGUILayout.Space(5);
                            EditorGUI.indentLevel++;
                            EditorGUI.indentLevel++;
                            materialEditor.ShaderProperty(useTriplanarWorldScale,styles.useTriplanarWorldScale);
                            materialEditor.ShaderProperty(useTriplanarBlend,styles.useTriplanarBlend);
                            EditorGUI.indentLevel--;
                            EditorGUI.indentLevel--;
                        }
                        EditorGUI.indentLevel--;
                        ...
                        EditorGUI.indentLevel++;
                        EditorGUI.BeginChangeCheck();
                        upom = (UsePixelDisplacement)mat.GetInt(kusePixelDisplacement);
                        upom = (UsePixelDisplacement)EditorGUILayout.EnumPopup(s_Styles.usePixelDisplacement,upom);
                        if (EditorGUI.EndChangeCheck())
                        {
                            switch (upom)
                            {
                                case UsePixelDisplacement.Off:
                                    mat.SetInt(kusePixelDisplacement, 0);
                                    mat.DisableKeyword("_USEPIXELDISPLACEMENT");
                                    break;
                                case UsePixelDisplacement.UsePOM:
                                    mat.SetInt(kusePixelDisplacement, 1);
                                    mat.EnableKeyword("_USEPIXELDISPLACEMENT");
                                    break;
                            }
                        }
                        if(upom == UsePixelDisplacement.UsePOM)
                        {
                            EditorGUILayout.Space(5);
                            EditorGUI.indentLevel++;
                            EditorGUI.indentLevel++;
                            materialEditor.ShaderProperty(usePixelDisplacementHeight,styles.usePixelDisplacementHeight);
                            materialEditor.ShaderProperty(usePixelDisplacementSteps,styles.usePixelDisplacementSteps);
