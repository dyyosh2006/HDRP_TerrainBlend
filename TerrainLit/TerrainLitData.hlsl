...
//sheep yang
float _useTriplanarWorldScale;
float _useTriplanarBlend;
...
void CustomTerrainLitShade(float2 uv,float2 xz,float2 xy,float2 yz,float3 Node_Blend, float worldScale,float3x3 tbn,float3 V,inout TerrainLitSurfaceData surfaceData);
...
void GetSurfaceAndBuiltinData(inout FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData RAY_TRACING_OPTIONAL_PARAMETERS)
{
    ...
    float3 posForTriplanar = GetAbsolutePositionWS(input.positionRWS);
    float3 nrm = input.tangentToWorld[2].xyz;
    float3 Node_Blend = pow(abs(nrm - 0.2), _useTriplanarBlend);
    Node_Blend /= (dot(Node_Blend, 1.0) + 0.00001);
    CustomTerrainLitShade(input.texCoord0.xy, posForTriplanar.xz ,posForTriplanar.xy ,posForTriplanar.zy ,Node_Blend,_useTriplanarWorldScale,input.tangentToWorld,V,terrainLitSurfaceData);
